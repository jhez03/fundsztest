...
/*function admin_redirect($url, $refresh = 'refresh') {
	redirect(ADMINURL.'/' . $url, $refresh);
}*/

function admin_redirect($url, $refresh = 'refresh') {

	//echo base_url().$url; die;
...
function getSiteFavicon() {
	...
	$logo = $ci->db->where('id', 1)->get(SITE)->row()->fav_icon;
...
function getUserId($useraddress,$column){
	
	$username = get_data(USERS, array('address'=>$useraddress))->row();
...
function encrypt_decrypt($action, $string) {
	...
	$secret_key 		= 	'20FunDZZWMLMC';

	$secret_iv 			= 	'20WFunDzCWMLMR';
	// hash
...
function insep_encode($value) {
    ...
    $secret_key 	= 	'TFuW2UnSZz0C';

    $secret_iv 		= 	'20FunZZZ20M';
...
function juego_id() {
	return FS::session()->userdata('juego_id');
}
...
function user_access($id='') {
	$ci = &get_instance();
if(empty($id)){
	// $redirect_access = get_data(ASSIGN, array('owner_id' => owner_id(), 'user_id' => admin_id(), 'gme_id' => gme_id()))->row();
...
function _GDOM() {
	...
	//$query    =   $ci->db->query('SELECT * FROM '.DOM.' ORDER BY RAND() LIMIT 1');

	$query = get_data(DOM, '', '', '1');

	//FS::db()->query('SELECT * FROM '.DOM.' LIMIT 1');
...
function adimg($admin_id,$profile_picture,$size) {
	...
	if ($profile_picture != 'user.png') {
		$img_url = base_url() . 'assets/img/site/img_admin/'	.	$admin_id 	. 	'/'	. 	$size . '_' . 	$profile_picture;
	} else {
		$img_url = base_url() . 'assets/img/site/img_admin/'	.	$profile_picture;
	}
	return $img_url;
}
...
function trigger_socket($data,$functionname)
{
	...
	else if($_SERVER['HTTP_HOST']=='fundsz.com' && $protocol=='http')
	{
		$host 	=	'fundsz.com';
	...
	else if($_SERVER['HTTP_HOST']=='fundsz.com' && $protocol=='https')
	{
		//$host 	=	'117.232.68.204';
 ...
 function coinmarket_value($value = '') {
				$eth_value = $value;
				
				$coin_usd_value	=	FS::Common()->getTableData(SITE, array('id' => '1'))->row()->coin_usd_value;

					if($coin_usd_value)
					{
						
						$return_value = $coin_usd_value* $eth_value;
						return $return_value;
	...
	function insertUserLevelHistory($user_id,$current_level) {

	$level_details 				=	@get_data(USER_L_P,array('level'=>$current_level),'price,no_of_days')->row();

	$hdata['user_id']			=	$user_id;

	$hdata['level_id']			=	$current_level;

	$hdata['level_price']		=	$level_details->price;

	$hdata['start_date']		=	date('Y-m-d H:i:s');

	$hdata['end_date']			=	date('Y-m-d H:i:s', strtotime("+".$level_details->no_of_days));
	...
	function updateuUserLevel($user_id, $current_level, $buyType,$level_prev, $level_current,$usd_earn_new,$usd_earn_old,$subscription_plan) {
		...
		if ($user_details) 
		{
			...
			if($buyType == "buy")
			{
				...
				$upd['extend_prev_status_100'] 			= 	1;
				$upd['extend_update_100'] 				= 	0;
				$upd['extend_prev_status_500'] 			= 	1;
				$upd['extend_update_500'] 				= 	0;
			}
			else
			{
				if($current_level=="1")
				{
				$upd['extend_prev_status_100'] 			= 	$level_current;
				$upd['extend_update_100'] 				= 	$level_prev+1;
				$upd['usd_earned_level1'] 				= 	$usd_earn_old+100;
				}
				else
				{
				$upd['extend_prev_status_500'] 			= 	$level_current;
				$upd['extend_update_500'] 				= 	$level_prev+1;
				$upd['usd_earned_level2'] 				= 	$usd_earn_old+500;
				}
			...
			$upd['subscription_plan'] 				= 	$subscription_plan;
	...
	function updateuUserMissingLevel($user_id, $current_level, $buyType,$level_exp) {

		$user_details 									=	@get_data(USERS,array('contract_id'=>$user_id),'user_levels')->row();
		...
		if ($user_details) 
		{
			...
			$levels[$current_level]['start_date']		= 	$start_date;

			$levels[$current_level]['end_date']			= 	$end_date;
	...
	function getuserdata($user_id,$column)
	{
			$present=@get_data("users",array("address"=>$user_id))->row();
