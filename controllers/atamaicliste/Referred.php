...
class Referred extends Front_Controller {
	...
	public function index()
	{
		if(!empty(juego_id()))
		{
			...
			$data['action']			=	"referred";

			$data['tree_data']		=	$this->getUserTreeData();

			$this->cview(strtolower(CI_MODEL).'/referred', $data);
		}
		else
		{
	...
	function getUserTreeData()
	{
		...
		$data 				=	FS::db()->query("select  *
from    (select CONCAT('ID: ' ,contract_id) as name, current_level as number, 'null' as parent,'User Info' as levelinfo,contract_id,affiliate_id,original_referrer from tfunds_trfdznreaseuer ORDER BY contract_id ASC) products_sorted,
